...
2. **Debug Menu** (hidden by default) — All video-production-calendar framework links (7 stages, agent files, tools). Only shown when the user clicks the **debug button** at the **bottom-right corner** of the page.
...
**Goal:** Create a template project that can be used by other projects at start — `video-production-calendar` v0.9
...
`video-production-calendar` is a **starter template** — other projects begin by copying it ("Use this template" on GitHub or `gh repo create <name> --template rifaterdemsahin/video-production-calendar`). If you are an LLM agent bootstrapping a new project from this template, follow these steps (see **SPEC-010** in `4_Formula/specs.md`):
...
|-------------|-------------------------------|------------------|
| `{{PROJECT_NAME}}` | `video-production-calendar` | `README.md`, `index.html` (title, hero), `2_Environment/supabase/config.toml` (project id) |
| `{{GITHUB_USER}}` | `rifaterdemsahin` | `README.md`, `index.html` (GitHub link), renderer local fallback |
| `{{REPO_NAME}}` | `video-production-calendar` | `README.md` Pages URL, `sitemap.xml`, `robots.txt` |
| `{{PAGES_URL}}` | `https://rifaterdemsahin.github.io/video-production-calendar/` | `README.md`, `sitemap.xml`, `robots.txt` |
